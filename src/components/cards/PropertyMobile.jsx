...
const TrimLeadingSpaces = ({ text }) => {
    const trimmedText = text ? text.replace(/^\s+/, '') : ''; // Use regex to remove leading spaces     
    return <span>{trimmedText}</span>;
};

const PropertyMobile = ({ property, numberOfProperty, width }) => {
	...
	return (
        <div>
            ...
            <Link to={`/properties/detail-property/${property.externalId}`} className="relative flex flex-col my-6 bg-white shadow-sm border border-slate-200 rounded-lg w-96 mobile-screen-property hover:scale-105 duration-200 ease-in-out hover:border-blue-500 hover:border-2" style={{ fontFamily: '"Josefin Sans",sans-serif', fontStyle: 'normal' }}>
                ...
                <div className="p-4">
                    <h6 className="relative bottom-5 mb-10 text-slate-800 text-base font-semibold" style={{ fontSize: '13px' }}>
                        {property?.title.substring(0,22)} <br />
                        <span className="border border-bg-color rounded-lg text-bg-color mx-auto px-4 py-0.5" style={{ fontSize: '11px' }}>
                            For {property.propertyType === "Event Space" ? "Rent" : property?.marketType}
                        </span>
                    </h6>

                    <p className="relative bottom-[55px] text-slate-600 leading-normal font-light">
                        <b className='font-semibold text-sm' style={{ color: '#488cf5' }}>{property?.currency} {formatNumberWithCommas(property?.amount) || 0}</b> {duration}
                    </p>

                    {/* {property.location.substring(0,25).trim()} */}

                    <p className="relative flex bottom-[50px] text-slate-600 leading-normal font-light">
                        <FaLocationDot  className='relative mr-1 text-xs' style={{ color: '#488cf5' }}/>
                        <span className="text-xs">{property.location.substring(0,30)}</span>
                    </p>
                    <div className="relative bottom-10 text-slate-600 mb-2 leading-normal font-light items-center property-amentity">
                        <hr className="w-48 h-1 mx-auto my-2 mt-2 bg-gray border-1 rounded md:my-10 dark:bg-gray property-lined"></hr>
                        <div className="flex relative right-7 text-sm property-amentity-icons">
                            {
                                property?.propertyType === "Event Space" ? (
                                    <>
                                        <div className="flex" style={{ fontSize: '12px' }}>
                                            ...
                                            <div className='mr-5'>
                                                <p className="relative flex text-slate-600 leading-normal font-light">
                                                    NR.&nbsp; <span className={` ${property?.IsNoise === true ? ' text-bg-color' : 'border-red-800 text-red-800'} rounded-full px-0.5 py-0.2`}>{ property?.IsNoise === true ? 'Yes' : 'No'}</span>
                                                </p>
                                            </div>

                                            <div className='mr-2'>
                                                <p className="relative flex text-slate-600 leading-normal font-light">
                                                    AN&nbsp; <span className={`${property?.isAdvancedNotice === true ? 'text-bg-color' : 'text-red-800'} rounded-full px-0.5 py-0.2`}>{ property?.isAdvancedNotice === true ? 'Yes' : 'No'}</span>
                                                </p>
                                            </div>
                                        </div>
                                    </>
...
PropertyMobile.propTypes = {
    ...
    numberOfProperty: PropTypes.number,
