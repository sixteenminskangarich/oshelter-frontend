...
import { editService, fetchService, getCategories } from '../../../../utils/request';
...
const EditService = () => {
    ...
    const {data: services,isLoading,status,} = useQuery({queryKey: ['services', { token, externalId }],queryFn: () => fetchService(token, externalId)});
	// Fetch categories
	const { data: category } = useQuery(['categories'], getCategories);

	const mutation = useMutation(
		(serviceData) => editService(serviceData, token, externalId),
		{
			onSuccess: (data) => {
    ...
    const { values, errors, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        initialValues: {
			...
			type_of_service_id: services?.TypeOfServiceId,
			photos: "",   
        },
												...
												<div>
													{
                                                        ...
                                                        ) : services?.images != null ? (
                                                            <img src={services?.images} alt="Featured" className="h-48 w-48 rounded-lg"/>
                                                        ) : (
